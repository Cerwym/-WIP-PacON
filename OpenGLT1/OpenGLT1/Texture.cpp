...
Texture::Texture(const std::string& strFileName) 
...
void Texture::LoadFile(const std::string& strFileName)
{
	// TODO: Consider using the ILUT library for easy loading and binding (DEVIL)
...
void Texture::ReleaseReference()
{
	...
	m_iRefCount--;
	if (m_iRefCount == 0)
		TextureManager::GetInstance()->ReleaseTexture(m_strTextName);
}
...
void Texture::SetColorKey(unsigned char Red, unsigned char Green, unsigned char Blue)
{
	...
	{
		if ( (m_TextData.pData[i]==Red) && (m_TextData.pData[i+1]==Green) && (m_TextData.pData[i+2]==Blue) )
